...
def update_grad_W_b_in_layer(A3, A2, A1, W3, W2, Z3, Z2, Z1, image, grad_W3, grad_W2, grad_W1, grad_b3, grad_b2, grad_b1):
    # layer n_y and n_h2
    for j in range(n_y):
        for k in range(n_h2):
            grad_W3[j][k] += (2 * (A3[j][0] - 2 * (image[1][j]))) * sigmoid_derivative(Z3[j][0]) * (A2[k][0])
            grad_b3[j][0] += (2 * (A3[j][0] - 2 * (image[1][j]))) * sigmoid_derivative(Z3[j][0])
    ...
    for batch in batches:
        ...
        for image in batch:
            ...
            grad_W3, grad_W2, grad_W1, grad_b3, grad_b2, grad_b1 = update_grad_W_b_in_layer(A3, A2, A1, W3, W2, Z3, Z2,
                                                                                            ...
                                                                                            grad_W1, grad_b3, grad_b2, grad_b1)
...
print("Accuracy :", number_of_correct_answer / (number_of_epochs * 100))
